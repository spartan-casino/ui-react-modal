...
class Store {
  ...
  _getDefaultValues = () => {
    return {
      ...
      vaultAssets: [
        ...
        {
          id: "ROCK",
          name: "TheMoon.finance",
          symbol: "Rock",
          description: "TheMoon.finance",
          vaultSymbol: "ROCK",
  ...
  stake = async (account, amount) => {
    ...
    const stakingContract = new web3.eth.Contract(
      config.stakeContractABI,
      config.stakeContractAddress
    );

    tokenInst.methods
      .allowance(account.address, config.stakeContractAddress)
      .call()
      .then(async function (allowance) {
        if (
          parseInt(allowance) <
          parseInt(web3.utils.toWei(amount.toString(), "ether"))
        ) {
          let approveTxAbi = tokenInst.methods
            .approve(
              config.stakeContractAddress,
          ...
          let approveRes = await web3.eth.sendTransaction(approveTransaction);
        }

        let stakeTxAbi = stakingContract.methods
          .stake(web3.utils.toWei(amount.toString(), "ether"))
          .encodeABI();

        let stakeTransaction = {
          to: config.stakeContractAddress,
        ...
        let stakeRes = await web3.eth.sendTransaction(stakeTransaction);
        return stakeRes;
      });
  };

  stakeInfo = async (account) => {
    const web3 = new Web3(window.web3.currentProvider);
    ...
    const result = await stakingContract.methods
      .getStakeHolderInfo(account.address)
      .call();

    return result;
  };

  stakeRewardInfo = async (account, date) => {
    const web3 = new Web3(window.web3.currentProvider);
    ...
    const result = await stakingContract.methods
      .forecastStakeHolderRewards(account.address, date)
      .call();

    return result;
  };

  isStakeholder = async (account) => {
    const web3 = new Web3(window.web3.currentProvider);
    ...
    const result = await stakingContract.methods
      .isStakeholder(account.address)
      .call();

    return result;
  };

  unstake = async (account) => {
    const web3 = new Web3(window.web3.currentProvider);
    ...
    let unstakeTxAbi = stakingContract.methods.unstake().encodeABI();
    ...
    let tx2 = await web3.eth.sendTransaction(unstakeTransaction);
  };
  ...
  _getAddressStats = (addressStatistics, asset, callback) => {
    try {
      ...
      const vault = addressStatistics.filter((stats) => {
        return (
          stats.vaultAddress.toLowerCase() ===
